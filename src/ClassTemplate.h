...
#line 4
    ...
    void render_header()
    {
      _serialize.text("template<typename DERIVED_T, typename DATA_T, typename SERIALIZER_T>\n"
             "struct "); _serialize.escape(data.class_name); _serialize.text("_t\n");
      if (!data.parent_class_name.empty())
      {
        _serialize.text("   : public "); _serialize.escape(data.parent_class_name); _serialize.text("_t<"); _serialize.escape(data.class_name); _serialize.text("_t<DERIVED_T, DATA_T, SERIALIZER_T>, DATA_T, SERIALIZER_T>\n");
      }
      _serialize.text("{\n");
      if (!data.parent_class_name.empty())
      {
        _serialize.text("  using _parent_t = "); _serialize.escape(data.parent_class_name); _serialize.text("_t<"); _serialize.escape(data.class_name); _serialize.text("_t, DATA_T, SERIALIZER_T>;\n"
               "  _parent_t& parent;\n");
      }
      _serialize.text("  DERIVED_T& child;\n"
             ...
             "  _serializer_t& _serialize;\n"
             "\n"
             "  "); _serialize.escape(data.class_name); _serialize.text("_t(DERIVED_T& derived, const DATA_T& data_, SERIALIZER_T& serialize):\n");
      if (!data.parent_class_name.empty())
      {
    ...
    void render_footer()
    {
      _serialize.text("  // ----------------------------------------------------------------------\n"
             ...
             "auto "); _serialize.escape(data.class_name); _serialize.text("(const DATA_T& data, SERIALIZER_T& serialize)\n"
             "  -> "); _serialize.escape(data.class_name); _serialize.text("_t<kiste::terminal_t, DATA_T, SERIALIZER_T>\n"
             "{\n"
