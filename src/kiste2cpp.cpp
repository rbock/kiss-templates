...
namespace
{
	const auto string_open = std::string(R"---(R"_____()---");
	const auto string_close = std::string(R"---()_____")---");
	...
	struct parse_context
	{
		...
		parse_context(std::istream& is_, std::ostream& os_, const std::string& filename_):
			is(is_),
			os(os_),
			filename{filename_}
		{}
	...
	auto parse_arg(parse_context& ctx, std::size_t pos)
		-> std::size_t
	{
	...
	auto parse_text_line(parse_context& ctx)
		-> void
	{
	...
	void parse_parent_class(parse_context& ctx, const std::string& line)
	{
		...
		ctx.parent_class_name = (nameEnd == line.npos)
			? line.substr(nameBegin)
	...
	void parse_class(parse_context& ctx, const std::string& line)
	{
		...
		const auto nameEnd = line.find_first_of(" \t", nameBegin);
		ctx.class_name = (nameEnd == line.npos)
			? line.substr(nameBegin)
			: line.substr(nameBegin, nameEnd - nameBegin);
		...
		while(ctx.is.good())
		{
			...
			else
			{
				...
				case '%': // cpp line
					...
					break;
				case '$': // opening / closing class
					{
						std::clog << "here we go: " << ctx.line.substr(pos_first_char + 1) << std::endl;
